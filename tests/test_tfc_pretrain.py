from torchchronos.datasets import TFCPaperDataset


def test_shapes_Gesture_train(tmp_path) -> None:
    """Test that the dataset returns the correct shapes."""

    dataset = TFCPaperDataset(name="Gesture", path=tmp_path, split="train")
...
def test_shapes_EMG_test(tmp_path) -> None:
    ...
    dataset = TFCPaperDataset(name="EMG", path=tmp_path, split="test")
